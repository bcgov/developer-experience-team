...
#  dd.sql("select questions.question_id, questions.title, from './questions_answers_comments.json' as questions where not list_contains(questions.tags, 'importer-ignore') and not list_contains(questions.tags, 'kibana')").show()
...
# dd.sql("select question_id, title, tags from  './questions_answers_comments.json' as questions where questions.question_id in (select question_id from (select unnest(questions.answers, recursive := true) from './questions_answers_comments.json' as questions) as answers where answers.body like '%<img%' UNION " \
       # "select question_id from './questions_answers_comments.json' as questions where questions.body like '%<img%')").show(max_rows=500)

# dd.sql("select question_id from (select unnest (answers.comments, recursive := true) from (select unnest(questions.answers, recursive := true) from './questions_answers_comments.json' as questions) as answers ) as comments where comments.body like '%<img%'").show()

# dd.sql("select * from (select answers.comments from (select unnest(questions.answers, recursive := true) from './questions_answers_comments.json' as questions) as answers) as comments where comments.edited = 'false'").show()

# dd.sql("select question_id, title from  './questions_answers_comments.json' as questions where questions.question_id in (select question_id from (select unnest(questions.answers, recursive := true) from './questions_answers_comments.json' as questions) as answers where answers.body like '%<img%')").show(max_rows=500)
# dd.sql("select question_id from './questions_answers_comments.json' as questions where questions.body like '%<img%'").show(max_rows=500)
...
dd.sql("Copy (" \
"select unnest(answers.comments).link from (select unnest(questions.answers, recursive:=true) from './questions_answers_comments.json' as questions where not list_contains(questions.tags, 'importer-ignore') and not list_contains(questions.tags, 'kibana')) as answers where answers.question_id " \
"UNION select unnest(questions.answers).link from './questions_answers_comments.json' as questions where questions.question_id and not list_contains(questions.tags, 'importer-ignore') and not list_contains(questions.tags, 'kibana') " \
"UNION select unnest(questions.answers).share_link from './questions_answers_comments.json' as questions where questions.question_id and not list_contains(questions.tags, 'importer-ignore') and not list_contains(questions.tags, 'kibana') " \
"UNION select questions.link from './questions_answers_comments.json' as questions where questions.question_id and not list_contains(questions.tags, 'importer-ignore') and not list_contains(questions.tags, 'kibana') " \
"UNION select questions.share_link from './questions_answers_comments.json' as questions where questions.question_id and not list_contains(questions.tags, 'importer-ignore') and not list_contains(questions.tags, 'kibana') " \
"UNION select unnest(questions.comments).link from './questions_answers_comments.json' as questions where questions.question_id and not list_contains(questions.tags, 'importer-ignore') and not list_contains(questions.tags, 'kibana') " \
...
# dd.sql("select unnest(answers.comments).link from (select unnest(questions.answers, recursive:=true) from './questions_answers_comments.json' as questions where ) as answers where answers.question_id").show()